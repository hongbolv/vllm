...
1. [概述](#1-概述)
2. [架构设计与核心抽象](#2-架构设计与核心抽象)
3. [量化方法注册与发现机制](#3-量化方法注册与发现机制)
4. [完整调用流程](#4-完整调用流程)
5. [主要量化方法详解](#5-主要量化方法详解)
   - 5.1 [AWQ 量化](#51-awq-activation-aware-weight-quantization)
   - 5.2 [GPTQ 量化](#52-gptq-量化)
   - 5.3 [FP8 量化](#53-fp8-量化)
   - 5.4 [CompressedTensors 统一框架](#54-compressedtensors-统一框架)
   - 5.5 [BitsAndBytes 量化](#55-bitsandbytes-量化)
   - 5.6 [GGUF 量化](#56-gguf-量化)
   - 5.7 [ModelOpt 量化](#57-modelopt-nvidia-量化框架)
   - 5.8 [TorchAO 量化](#58-torchao-量化)
   - 5.9 [Quark 量化框架](#59-quark-量化框架)
   - 5.10 [其他量化方法](#510-其他量化方法)
6. [MoE 模型量化](#6-moe-模型量化)
7. [KV Cache 量化](#7-kv-cache-量化)
8. [工具层与内核支持](#8-工具层与内核支持)
9. [量化方法对比总结](#9-量化方法对比总结)
10. [关键发现与洞察](#10-关键发现与洞察)

---

## 1. 概述

vLLM 是一个高性能的大语言模型推理引擎，其量化子系统是其核心组件之一。该系统实现了 **30+ 种量化方法**，覆盖了从 2-bit 到 8-bit 的各种精度，支持整数量化和浮点量化两大类技术路线。量化代码位于：
...
### 5.1 AWQ (Activation-aware Weight Quantization)

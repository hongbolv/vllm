...
5. [Backend 选择机制](#5-backend-选择机制)
   - 5.1 [NVIDIA CUDA 优先级](#nvidia-cuda)
   - 5.2 [AMD ROCm 优先级](#amd-rocm)
   - 5.3 [Intel XPU / CPU 优先级](#intel-xpu)
...
| 指标 | 数值 |
|------|------|
| 标准 Attention 后端数 | 8（FlashAttention/FlashInfer/Triton/ROCm/AITER/Flex/CPU 等） |
| MLA 专用后端数 | 10（FlashMLA/FlashInfer MLA/CUTLASS MLA/Triton MLA 等） |
